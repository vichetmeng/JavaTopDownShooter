 ...
 * @version 1.0
 */
public class ArmorCodexDetailsScene implements Initializable {
	private static final Controller controller = Controller.getInstance();

	@FXML
	private Label armorNameLabel;
	@FXML
	private ImageView armorImageImageView;
	@FXML
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		armor = controller.getSelectedCodexArmor();
		if (armor != null) {
			armorNameLabel.setText(armor.getName());
            armorImageImageView.setImage(armor.getImage());
            
            levelLabel.setText("Rarity Level: " + String.valueOf(armor.getRarity()));
            if (armor.getRarity() >= 5.0)
                levelImageView.setImage(new Image(new File(MainView.FIVE_STAR_PATH).toURI().toString()));
            else if (armor.getRarity() < 5.0 && armor.getRarity() >= 4.0)
                levelImageView.setImage(new Image(new File(MainView.FOUR_STAR_PATH).toURI().toString()));
            else if (armor.getRarity() < 4.0 && armor.getRarity() >= 3.0)
                levelImageView.setImage(new Image(new File(MainView.THREE_STAR_PATH).toURI().toString()));
            else if (armor.getRarity() < 3.0 && armor.getRarity() >= 2.0)
            ...
            ratingLabel.setText("Armor Rating: " + String.valueOf(armor.getArmorRating()));
            worthLabel.setText("Worth: " + String.valueOf(armor.getWorth()) + " gold");
            descriptionTextArea.setText(armor.getDescription());
		} else
			armorNameLabel.setText("An unknown error has occurred.");
	}
